...
async function main() {
  console.log("Deploying DepartmentVoting contract with account 0 as admin...");

  // Use account 0 as the admin (we know its private key and it has funds)
  const [adminAccount] = await hre.ethers.getSigners();
  const adminAddress = adminAccount.address;
  
  console.log("Deploying with admin address:", adminAddress);
  console.log("Admin balance:", hre.ethers.utils.formatEther(await adminAccount.getBalance()), "ETH");

  // Deploy the contract using account 0
